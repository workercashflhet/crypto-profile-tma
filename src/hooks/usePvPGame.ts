...
export const usePvPGame = () => {
  ...
  const getPlayerBetUsd = useCallback((player: PvPPlayer): number => {
    ...
    return (tonBets * prices.ton) + (starsBets * 0.013);
  }, []);
  ...
  const addBotPlayer = useCallback(() => {
    setCurrentRound(prev => {
      ...
      const availableBots = MOCK_PLAYERS.filter(bot => !prev.players.find(p => p.userId === bot.userId));
      if (availableBots.length === 0) return prev;

      const randomBot = { ...availableBots[Math.floor(Math.random() * availableBots.length)] };
      const randomBetAmount = Math.floor(Math.random() * 50) + 5;
      const botCurrency: CurrencyType = Math.random() > 0.3 ? 'ton' : 'stars';

      const playerColors = prev.players.map(p => p.color);
      const availableColors = PLAYER_COLORS.filter(c => !playerColors.includes(c));
      const botColor = availableColors.length > 0 ? availableColors[Math.floor(Math.random() * availableColors.length)] : PLAYER_COLORS[Math.floor(Math.random() * PLAYER_COLORS.length)];

      const botBets: PlayerBet[] = [{ amount: randomBetAmount, currency: botCurrency, timestamp: Date.now() }];
      ...
      const totalTon = updatedPlayers.reduce((sum, p) => sum + p.bets.filter(b => b.currency === 'ton').reduce((s, b) => s + b.amount, 0), 0);
      const totalStars = updatedPlayers.reduce((sum, p) => sum + p.bets.filter(b => b.currency === 'stars').reduce((s, b) => s + b.amount, 0), 0);

      return { ...prev, players: updatedPlayers, totalPoolTon: totalTon, totalPoolStars: totalStars, status: prev.players.length === 0 ? 'active' : prev.status };
    });
  }, []);

  useEffect(() => {
    if (!currentRound || currentRound.status !== 'active') { if (botsTimerRef.current) { clearInterval(botsTimerRef.current); botsTimerRef.current = null; } return; }
    botsTimerRef.current = setInterval(() => { addBotPlayer(); }, 4000 + Math.random() * 3000);
    return () => { if (botsTimerRef.current) { clearInterval(botsTimerRef.current); botsTimerRef.current = null; } };
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setCurrentRound(prev => {
        ...
        if (newTimeLeft <= 0) {
          if (prev.players.length === 0) return { ...prev, timeLeft: ROUND_DURATION, status: 'waiting' };
  ...
  const placeBet = useCallback((amount: number, currency: CurrencyType): boolean => {
    ...
    if (currency === 'ton') {
      if (!hasEnoughTon(amount)) { setError(`Insufficient TON balance. You have ${balance.ton.toFixed(1)} TON`); setTimeout(() => setError(null), 3000); return false; }
      withdrawTon(amount);
    } else {
      if (!hasEnoughStars(amount)) { setError(`Insufficient Stars balance. You have ${balance.stars.toFixed(0)} Stars`); setTimeout(() => setError(null), 3000); return false; }
      ...
      if (existingPlayerIndex >= 0) {
        updatedPlayers = prev.players.map((p, i) => {
          if (i === existingPlayerIndex) {
            ...
            return { ...p, bets: updatedBets, totalBet: updatedBets.reduce((sum, b) => sum + b.amount, 0), currency };
          }
          return p;
        });
      } else {
        const playerColors = prev.players.map(p => p.color);
        const availableColors = PLAYER_COLORS.filter(c => !playerColors.includes(c));
        const assignedColor = availableColors.length > 0 ? availableColors[Math.floor(Math.random() * availableColors.length)] : PLAYER_COLORS[Math.floor(Math.random() * PLAYER_COLORS.length)];
        updatedPlayers = [...prev.players, { userId: user.id, username: user.username || `user_${user.id}`, firstName: user.firstName, avatar: user.photoUrl || undefined, bets: [newBet], totalBet: amount, currency, color: assignedColor }];
      }
  ...
  const calculateSegments = useCallback(() => {
    ...
    return currentRound.players.map((player) => {
      ...
      return { color: player.color, percentage: percentage * 100, player, startAngle, endAngle };
    });
  ...
  const spinWheel = useCallback(() => {
    ...
    setRotationAngle(360 * 8 + (360 - stopAngle));
    let winnerPlayer: PvPPlayer | null = null;
    for (const segment of segments) { if (stopAngle >= segment.startAngle && stopAngle < segment.endAngle) { winnerPlayer = segment.player; break; } }
    if (!winnerPlayer && segments.length > 0) winnerPlayer = segments[0].player;
    setTimeout(() => {
      if (winnerPlayer) {
        setWinner(winnerPlayer);
        setCurrentRound(prev => prev ? { ...prev, status: 'finished', winner: winnerPlayer! } : prev);
        if (winnerPlayer.userId === user?.id && currentRound) { depositTon(currentRound.totalPoolTon); depositStars(currentRound.totalPoolStars); }
        autoResetRef.current = setTimeout(() => { initRound(); }, AUTO_RESET_DELAY);
      }
      setIsSpinning(false);
    }, 5000);
  }, [currentRound, isSpinning, calculateSegments, user?.id, depositTon, depositStars, initRound]);

  const resetRound = useCallback(() => {
    if (timerRef.current) { clearInterval(timerRef.current); timerRef.current = null; }
    if (botsTimerRef.current) { clearInterval(botsTimerRef.current); botsTimerRef.current = null; }
    if (autoResetRef.current) { clearTimeout(autoResetRef.current); autoResetRef.current = null; }
    initRound();
  }, [initRound]);

  useEffect(() => { initRound(); return () => { if (timerRef.current) clearInterval(timerRef.current); if (botsTimerRef.current) clearInterval(botsTimerRef.current); if (autoResetRef.current) clearTimeout(autoResetRef.current); }; }, [initRound]);

  const getTotalPool = useCallback(() => {
    if (!currentRound) return '0';
    const parts: string[] = [];
    if (currentRound.totalPoolTon > 0) parts.push(`${currentRound.totalPoolTon} TON`);
    if (currentRound.totalPoolStars > 0) parts.push(`${currentRound.totalPoolStars} ⭐`);
    return parts.join(' + ') || '0';
  }, [currentRound]);
