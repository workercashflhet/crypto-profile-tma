...
async function getOrCreateGameState() {
    let state = await (0, redis_1.getGameState)();
    if (!state) {
        state = createNewRound();
        await (0, redis_1.setGameState)(state);
    }
    return state;
}
        ...
        try {
            ...
            switch (action) {
                case 'place_bet': {
                    ...
                    const newBalance = await (0, redis_1.updatePlayerBalance)(userId, tonDelta, starsDelta);
                    ...
                    state.totalPoolTon = state.players.reduce((sum, p) => sum + p.bets.filter(b => b.currency === 'ton').reduce((s, b) => s + b.amount, 0), 0);
                    state.totalPoolStars = state.players.reduce((sum, p) => sum + p.bets.filter(b => b.currency === 'stars').reduce((s, b) => s + b.amount, 0), 0);
                    ...
                    await (0, redis_1.setGameState)(state);
                    await (0, redis_1.addPlayerBet)(userId, bet);
                    return res.status(200).json({
                        success: true,
                        data: state
                    });
                ...
                case 'spin': {
                    ...
                    await (0, redis_1.updatePlayerBalance)(winner.userId, state.totalPoolTon, state.totalPoolStars);
                    await (0, redis_1.addRoundHistory)({
                ...
                case 'reset': {
                    ...
                    await (0, redis_1.setGameState)(newState);
                    return res.status(200).json({
                        success: true,
                        data: newState
                    });
                ...
                case 'get_history': {
                    ...
                    const history = await (0, redis_1.getRoundHistory)(limit);
                    return res.status(200).json({
                        success: true,
                        history
                    });
